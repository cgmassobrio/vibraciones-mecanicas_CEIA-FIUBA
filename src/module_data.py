...
class DataSintetic:
    ...
    def pde_T(self, N_pde, LHS_seed):
        ...
        F1_pde = torch.zeros((N_pde, 1), device=self.device)
        F2_pde = (torch.cos(A_pde[:,1:2]) - (np.pi/2)*torch.sin(A_pde[:,0:1])*torch.cos(A_pde[:,1:2])).to(self.device)
        D_pde = torch.cat([F1_pde, F2_pde], dim=1)
    ...
    def bc_l_T(self, N_bc_l, bc_l_seed):
        ...
        W_bc_l = torch.zeros((N_bc_l, 1), device=self.device)
        Psi_bc_l = torch.zeros((N_bc_l, 1), device=self.device)
        D_bc_l = torch.cat([W_bc_l, Psi_bc_l], dim=1)
    ...
    def bc_u_T(self, N_bc_u, bc_u_seed):
        ...
        W_bc_u = torch.zeros((N_bc_u, 1), device=self.device)
        Psi_bc_u = torch.zeros((N_bc_u, 1), device=self.device)
        D_bc_u = torch.cat([W_bc_u, Psi_bc_u], dim=1)
    ...
    def ic_T(self, N_ic, ic_seed):
        ...
        W_ic = (torch.sin(A_ic[:,0:1])*np.pi/2).to(self.device)
        W_t_ic = torch.zeros((N_ic, 1), device=self.device)
        Psi_ic = ((np.pi/2)*torch.cos(A_ic[:,0:1]) + (A_ic[:,0:1]-np.pi/2)).to(self.device)
        Psi_t_ic = torch.zeros((N_ic, 1), device=self.device)
        D_ic = torch.cat([W_ic, W_t_ic, Psi_ic, Psi_t_ic], dim=1)
    ...
    def ds_train_T(self, N_pde, N_bc_l, N_bc_u, N_ic, LHS_seed, bc_l_seed, bc_u_seed, ic_seed, 
                   ...
                   isData=False, isP=True, save_data=False, save_data_path=None):
        '''
       ...
       # Initialization of datasets list
        ...
        if isData:
            ...
            ds.append(self.inv_data(N_data, omega, sensor_loc, inv_seed))
