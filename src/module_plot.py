...
class Plot():
    ...
    def plot_predict3D(self, data_dict, figure_path, level_cont=None, title=None):
        ...
        heatmap = ax.plot_surface(T_m, X_m, O, cmap='rainbow')
        cbar = fig.colorbar(heatmap, ax=ax, orientation='vertical')
        ax.set_xlabel('T')
        ax.set_ylabel('X')
        ax.set_title(title)
    ...
    def plot_train(self, data_dict, figure_path, p_param=None, x_i=None, isData=False, title=None):
        ...
        ax.scatter(A_pde_cpu[:, 1:2], A_pde_cpu[:, 0:1], s=4, c='orangered', label=r'$N_{dom}$='f'{len(A_pde_cpu)}')
        ...
        el = ['Fa', '-', 'Fa', '-', 'Fa']
        color_el = ['lime', 'green', 'deepskyblue', 'navy', 'brown']
        if isData:
        ...
        if np.any(p_param):
            ...
            for i in idx_nz:
                ax.hlines(y=x_i[i], xmin=0, xmax=A_pde_cpu[:, 1:2].max(), lw=2, colors=color_el[i], label=f'EP {el[i]}')
    ...
    def loss_graphics(self, loss_train_dict, figure_path):
        ...
        # num_colors = len(loss_train_dict)
