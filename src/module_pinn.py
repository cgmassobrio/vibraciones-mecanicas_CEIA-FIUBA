...
class PINN_Model(nn.Module):
    ...
    Input:
    - model_nn: JSON file containing dictionaries that group the parameters needed to work with the PINN model.
    '''
    ...
    def governingEquationResidue(self, A, B):
        ...
        # eq_1 = -Psi_x + W_xx - self.gamma_bs*self.R02*W_tt + self.gamma_bs*self.R02*F_a
        # eq_2 = W_x + self.gamma_bs*self.R02*Psi_xx - Psi - self.gamma_bs*self.R02**2*Psi_tt

        # eq_1 = -Psi_x + W_xx - W_tt + F_a
        # eq_2 = W_x + Psi_xx - Psi - Psi_tt
    ...
    def bclResidue(self, A, B):
        ...
        # eq_1 = W - (W_x - Psi)/(self.gamma_bs*self.R02*self.K_L) + self.P_av*W_x
        ...
        if self.K_L == 0.0 and self.K_tL == 0.0:
            ...
            # First order derivatives of W respect to A
            W_x = torch.autograd.grad(W, A, grad_outputs=torch.ones_like(W), allow_unused=True, create_graph=True)[0][:,0:1]
            eq_1 = W*self.K_L - (W_x - Psi)/(self.gamma_bs*self.R02)
            eq_2 = Psi
    ...
    def bcuResidue(self, A, B):
        ...
        # eq_1 = W + (W_x + Psi)/(self.gamma_bs*self.R02*self.K_R) + self.P_av*W_x
        # eq_2 = Psi + Psi_x/self.K_tR

        if self.K_R == 0.0 and self.K_tR == 0.0:
            eq_1 = W
            eq_2 = Psi
        
        elif self.K_R != 0.0 and self.K_tR == 0.0:
            # First order derivatives of W respect to A
            W_x = torch.autograd.grad(W, A, grad_outputs=torch.ones_like(W), allow_unused=True, create_graph=True)[0][:,0:1]
            eq_1 = W*self.K_R + (W_x - Psi)/(self.gamma_bs*self.R02)
            eq_2 = Psi
    ...
    def icResidue(self, A, B):
        ...
        return torch.cat([W, Psi, W_t, Psi_t], dim=1)
...
class Fit_model:
    ...
    def __init__(self, param_data, param_model):
        ...
        self.model = PINN_Model(param_data, param_model).to(self.device)
    ...
    def train(self, epochs, datasets, loss_fn, optimizer, save_model=False, model_path=None, metric_path=None):
        '''
        Inputs
        ...
        train_keys = ['Overall', 'PDE', 'BCL', 'BCU', 'IC']
    ...
    def saveModel(self, model_path, metric_path):
        ...
        # Dictionary with recorded losses: total, domain, BC (boundary conditions), and IC (initial conditions)
